...
import ba.unsa.etf.zavrsniRad.data.*
import ba.unsa.etf.zavrsniRad.data.collections.Note
import ba.unsa.etf.zavrsniRad.data.requests.AddOwnerRequest
import ba.unsa.etf.zavrsniRad.data.requests.DeleteNoteRequest
        ...
        authenticate {
            post{
                ...
                if(!checkIfUserExists(request.owner)){
                    call.respond(
                        OK,SimpleResponse(false,"NO user with this email exists!")
                    )
                    return@post
                }
                if(isOwnerOfNote(request.noteID,request.owner)){
                    call.respond(
                        OK,SimpleResponse(false,"User is already an owner of this note!")
                    )
                    return@post
                }
